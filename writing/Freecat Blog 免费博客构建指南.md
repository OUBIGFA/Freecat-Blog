...
description: 一套本地优先且深度定制的个人博客方案：通过 GitHub 云备份，并用 Cloudflare Pages 或 Vercel 免费部署，让你跳过无谓的格式折腾，回归纯粹的写作与阅读。
...
1. 你在本地写 `Markdown`
2. 用 `GitHub` 做云备份和版本管理
3. 用 `Cloudflare Pages` 或 `Vercel` 自动构建并发布成网站
