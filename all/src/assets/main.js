document.addEventListener('DOMContentLoaded', () => {
    ...
    const { escapeHtml, processTitleHtml, renderTagSpan } = shared;

    const codeCopy = window.FreecatCodeCopy;
    if (!codeCopy || typeof codeCopy.init !== 'function') {
        throw new Error('FreecatCodeCopy not loaded - ensure code-copy.js loads before main.js');
    }
    ...
    function initFloatingNavButtons() {
        const floatingNav = window.FreecatFloatingNav;
        if (!floatingNav || typeof floatingNav.init !== 'function') {
            throw new Error('FreecatFloatingNav not loaded - ensure floating-nav.js loads before main.js');
        }
