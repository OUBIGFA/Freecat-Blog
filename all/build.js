 ...
 *
 * 整体流程�?
 *   1. 清理 dist 并搬�?assets / image
 *   2. 加载站点 / 社交 / 关于页面三份配置（Control/*.md�?
 *   3. 准备模板引擎（注�?partials + tailwind config + SITE_*�?
 *   4. 依次生成：post 详情�?�?分页首页 �?all.html �?search.html + 索引 �?about.html �?sitemap/robots
 *
 * 各步骤的实际逻辑都拆�?build/*.js 中，本文件仅做编排�?
 */
...
// 顶栏「查看标签」菜单：构建期一次性算好并预渲染成静�?HTML，注入每个页面的 header�?
// 这样客户端首次点击只需切换一�?CSS 类即可展开（零网络、零计算、实时响应）�?
// 不再首点拉取并解析整�?search-index.json�?
...
if (siteConfig.show_recent_posts === true) {
    ...
    // 入场动画延迟：h3 450ms 起步（接�?hero 阶梯 0�?50�?00），每条 li 再加 75ms
    ...
    const itemsHtml = recentPosts.map((post, i) => {
        ...
        return `
                            <li class="py-3 animate-fade-in-up" style="animation-delay: ${delay}ms">
                                <a href="${safeLink}" class="block text-[13px] font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors duration-150 line-clamp-2">
                                    ${safeTitle}
                                </a>
                            </li>`;
    }).join('\n');
    ...
    recentPostsSidebarInnerHtml = `
                        <div class="flex flex-col flex-shrink-0 mt-8">
                            <h3 class="text-[13px] font-semibold tracking-wider text-slate-600 dark:text-slate-300 mb-4 animate-fade-in-up" style="animation-delay: ${HEADING_DELAY}ms">
                                最近更�?
                            </h3>
    ...
    // 首页 / 搜索页改版后：最近更新直接渲染到左侧栏底部容器内�?    // 不再需要额外的 absolute/sticky shell 包裹，直接复�?inner 结构�?    recentPostsSidebarHomeWrapperHtml = recentPostsSidebarInnerHtml.replace(' mt-8', '').trim();
...
// ===== 8. 编译 Tailwind（必须最后做，扫描的是已生成�?dist HTML + 拼装 HTML 字符串的 build/ JS�?=====
...
buildTailwindCss({
    ...
    // ===== 9. Minify HTML / JS（必须在 Tailwind 之后，因�?Tailwind 会扫描已生成�?HTML�?=====
    if (process.env.NO_MINIFY !== '1') {
        console.log('🗜�? Minifying HTML & JS...');
